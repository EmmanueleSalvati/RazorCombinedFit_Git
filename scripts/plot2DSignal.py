...
def binnedHisto2D(tree, binsX, binsY, label='', efficiency=1, mass=400):
    """Make histogram"""

    histo = rt.TH2F("histo"+label, "histo"+label,
...
def binnedHisto1D(tree, binsX, varName, label='', xsec=None, eff=None):
    ...
    histo = rt.TH1F("histo1D"+varName+label, "histo1D"+varName+label,
                    len(binsX)-1, binsX)
    ...
    for i_binx in range(0, len(binsX) - 1):
        binx = binsX[i_binx]
        binx_p1 = binsX[i_binx+1]
        numEntries = (tree.reduce("%s >= %s && %s <= %s" %
                        (varName, binx, varName, binx_p1)).numEntries())
        Ngen += numEntries
...
if __name__ == '__main__':
    ...
    # ######### binning def
    # MRbins = array('d', [450, 490., 530., 575., 625., 700., 900., 4000.])
    # Rsqbins = array('d', [0.10, 0.12, 0.14, 0.17, 0.23, 0.30, 0.40, 0.7, 1.5])

    MRbins = array('d', [450., 470., 510., 550, 600., 800., 1000.,
                         4000.])
    Rsqbins = array('d', [0.10, 0.15, 0.20, 0.26, 0.35, 0.50, 1.0, 1.5])
    ...
    # ##### SAMPLES
    directory = './'  # Datasets/T2tt/'
    if box == 'BJetHS' or box == 'BJetLS':
        label = '_MR450.0_R0.316227766017_'
    else:
        label = '_MR350.0_R0.282842712475_'

    mStopmin = mStops
    mStopmax = mStops+nmStops*275
    ...
    # for mStop in range(mStopmin, mStopmax, 275):
    for mStop in range(150, 175, 25):  # 825, 25):
        filename = directory + 'SMS-T2tt_mStop-Combo_mLSP_' + str(mLSP) +\
            '.0_8TeV-Pythia6Z-Summer12-START52_V9_FSIM-v1-SUSY' + label +\
        ...
        if mStop > mLSP+175.:
            massivemDelta[mStop] = sqrt((mStop*mStop -
                (mLSP - 176.)*(mLSP - 176.)) *
                (mStop*mStop - (mLSP + 176.)*(mLSP + 176.))) / (1.*mStop)
...
# #          text.Draw("SAME")
        ...
        histoRsq.SetTitle("Rsq, %s box mStop : %sGeV, mLSP : %s GeV;Rsq;" %
                          (box, str(mStop), str(mLSP)))
        ...
        histoMR.SetTitle("MR, %s box mStop : %sGeV, mLSP : %s GeV;MR(GeV);" %
                         (box, str(mStop), str(mLSP)))
    ...
    c1D = rt.TCanvas("2Dplots/Rsq_mLSP%s_box%s.png" %
                     (mLSP, box), "Rsq_mLSP%s_box%s.png" % (mLSP, box))
    c1D.cd()
    c1D.SetLogy()
    ...
    c1D1 = rt.TCanvas("2Dplots/MR_mLSP%s_box%s.png" %
                      (mLSP, box), "MR__mLSP%s_box%s.png" % (mLSP, box))
    c1D1.cd()
    c1D1.SetLogy()
    ...
    for key, h in sorted(histoMRs.iteritems()):
        if n == 0:
            ...
            legMR.AddEntry(h, "mStop="+str(key)+"GeV", "lep")
        else:
            h.SetLineColor(rt.kBlack+n)
            h.Draw("SAME")
            legMR.AddEntry(h, "mStop="+str(key)+"GeV", "lep")
