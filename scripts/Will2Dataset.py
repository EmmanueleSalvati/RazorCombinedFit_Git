...
class BJetBox(object):
    ...
    def __call__(self, tree):
        return tree.hadBoxFilter and tree.nJet >= 6 and tree.nCSVM > 0 and\
            tree.MR >= MR_CUT_HAD and tree.RSQ >= RSQ_CUT and tree.nMuonTight < 1\
            and tree.nElectronTight < 1 and not tree.isolatedTrack10Filter and\
            tree.nMuonLoose < 1 and tree.nElectronLoose < 1 and tree.hadTriggerFilter
...
class BVetoBoxHS(object):
    """The BVetoHS box used in the analysis"""
    def __init__(self, dumper):
        self.name = 'BVetoHS'
        self.dumper = dumper
...
class MuBox(object):
    ...
    def __call__(self, tree):
        return tree.muBoxFilter and tree.nJetNoLeptons == 4 and tree.muTriggerFilter\
            and tree.nCSVM > 0 and tree.MR >= MR_CUT_LEP and tree.RSQ >= RSQ_CUT and\
            tree.nMuonTight == 1 and tree.nElectronTight == 0 and tree.nMuonLoose == 1 and\
            tree.nElectronLoose == 0 and not tree.isolatedTrack10LeptonFilter
...
class EleBox(object):
    ...
    def __call__(self, tree):
        return tree.eleBoxFilter and tree.nJetNoLeptons > 3 and tree.eleTriggerFilter and\
            tree.nCSVM > 0 and tree.MR >= MR_CUT_LEP and tree.RSQ >= RSQ_CUT and tree.nMuonTight == 0 and\
            tree.nElectronTight == 1 and tree.nMuonLoose == 0 and tree.nElectronLoose == 1 and not tree.isolatedTrack10LeptonFilter


class SelectBox(object):
    """Tells you which box this was"""
    def __init__(self, tree):
        self.selectors = {boxMap['BJetLS']: BJetBoxLS(CalcBDT(tree)), boxMap['BJetHS']: BJetBoxHS(CalcBDT(tree)),
                          boxMap['Ele']: EleBox(None), boxMap['Mu']: MuBox(None)}
        self.tree = tree
